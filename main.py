...
aniket72 = commands.Bot(description='SELFBOT CREATED BY Punit </3',
                           command_prefix='.',
                           self_bot=True,
                           intents=intents)
status_task = None
    ...
    async with aiohttp.ClientSession() as session:
        for link in links:
            ...
            if promo_code:
                ...
                await ctx.send(f'# __Punit S3LFB0Ƭ__\n`-` **INVALID LINK** : `{link}`')
...
async def ltcprice(ctx):
    ...
    if response.status_code == 200:
        ...
        await ctx.send("# __Punit S3LFB0Ƭ__\n`-` **FAILED TO FETCH**")
